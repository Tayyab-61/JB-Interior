...
export function PayRuns() {
  ...
  const handleSort = (key) => {
    ...
    const sortedPayRuns = [...payRuns].sort((a, b) => {
      if (key === 'payrollCost') {
        ...
        <div className="flex items-center space-x-2">
          <div className="relative inline-block">
            <select className="appearance-none w-[140px] cursor-pointer inline-flex items-center rounded-md bg-gradient-to-r from-violet-500 to-indigo-500 px-4 py-2 text-sm font-medium text-white border-none focus:ring-2 focus:ring-violet-400 focus:ring-offset-2 hover:from-violet-600 hover:to-indigo-600 transition-all duration-200 shadow-md hover:shadow-lg">
              ...
              <option value="paid">Paid PayRuns</option>
              <option value="pending">Pending PayRuns</option>
            </select>
        ...
        <table className="min-w-full divide-y divide-indigo-100">
          <thead className="border-b-2 border-indigo-100">
            <tr>
              ...
              <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-black uppercase tracking-wider">Pay Date</th>
              <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-black uppercase tracking-wider">Details</th>
              <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-black uppercase tracking-wider">
                <div className="flex items-center gap-2 cursor-pointer group" onClick={() => handleSort('payrollCost')}>
                  Payroll Cost
          ...
          <tbody className="bg-white divide-y divide-indigo-50">
            {payRuns.map((payRun) => (
              <tr key={payRun.id} className="hover:bg-gradient-to-r hover:from-indigo-50 hover:to-violet-50 transition-colors duration-150">
                <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                  <input type="checkbox" className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"/>
                </td>
                <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-black font-medium">{payRun.id}</td>
                <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-black">{payRun.details}</td>
                <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-black">{payRun.payrollCost}</td>
                <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                  <span
